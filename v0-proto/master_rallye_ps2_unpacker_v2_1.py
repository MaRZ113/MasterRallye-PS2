...
def main():
    ap = argparse.ArgumentParser(description="Master Rallye PS2 unpacker v2.1 / xmlish analyzer")
    ...
    p3 = sub.add_parser("carve-window", help="Carve a raw byte window from a source file")
    p3.add_argument("src", type=Path)
    p3.add_argument("offset", type=lambda x: int(x, 0))
    p3.add_argument("size", type=lambda x: int(x, 0))
    ...
    elif args.cmd == "carve-window":
        carve_window(args.src, args.offset, args.size, args.out)
        print(f"Wrote {args.out} ({args.size} bytes from 0x{args.offset:X})")
