...
def main():
    ap = argparse.ArgumentParser(description='BX v114 quarantine bucket splitter for 425425**')
    sub = ap.add_subparsers(dest='cmd', required=True)

    p = sub.add_parser('split-quarantine-bucket')
    p.add_argument('tng_path', type=Path)
    p.add_argument('out_dir', type=Path)
    p.add_argument('--sig7', type=str, default='0000010c425425')
    p.add_argument('--prev', type=str, default='none')
    p.add_argument('--next', type=str, default='0D@552')
    ...
    ns = ap.parse_args()
    if ns.cmd != 'split-quarantine-bucket':
        raise SystemExit(1)
    ...
    summary = []
    summary.append('BX v114 quarantine bucket splitter')
    summary.append('=================================')
    ...
    for row in stable_rows:
        proposed_rules.append({
            'name': f'quarantine_552_{row["sig8"][-2:]}_{row["body_prefix"]}',
            'sig7': ns.sig7,
            'prev': ns.prev,
            'next': ns.next,
            'member': {
