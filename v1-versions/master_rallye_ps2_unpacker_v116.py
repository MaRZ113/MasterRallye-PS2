...
def score_bucket_match(hit, bucket):
    # Strict scoring:
    # 0 = no bucket match
    # 1 = bucket prev/next matches, but no member for sig8
    # 2 = member sig8 matches
    # 3 = member + body_prefix matches
    # 4 = member + body_prefix + tail_sig8 matches (strongest)
    ...
    score = 2
    if expected.get('body_prefix') and hit['body_prefix'] == expected['body_prefix']:
        score = 3
    elif expected.get('body_prefix'):
        return score
...
def main():
    ap = argparse.ArgumentParser(description='BX v116 strict 425425 framework extractor')
    sub = ap.add_subparsers(dest='cmd', required=True)

    p = sub.add_parser('extract-425425-framework-strict')
    ...
    ns = ap.parse_args()
    if ns.cmd != 'extract-425425-framework-strict':
        raise SystemExit(1)
    ...
    summary = []
    summary.append('BX v116 strict 425425 framework extractor')
    summary.append('=========================================')
    ...
    for hit in all_hits:
        ...
        # Only promote to clean framework when member-specific evidence is present.
        # score >= 3 means member sig8 + body_prefix matched.
        if best_bucket is not None and best_score >= 3:
    ...
    for bucket in BUCKET_REGISTRY:
        ...
        with (bdir / 'extract_manifest.csv').open('w', encoding='utf-8', newline='') as f_csv:
            fieldnames = ['bucket_name','off_hex','sig8','body_md5','body_prefix','prev_key','next_key','tail_sig8','match_score','body_prefix_match','tail_sig8_match']
    ...
    with (qdir / 'quarantine_manifest.csv').open('w', encoding='utf-8', newline='') as f_csv:
        fieldnames = ['off_hex','sig8','body_md5','body_prefix','prev_key','next_key','tail_sig8','best_bucket_name','best_score']
        w = csv.DictWriter(f_csv, fieldnames=fieldnames)
        w.writeheader()
