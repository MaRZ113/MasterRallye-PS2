...
def main():
    ap = argparse.ArgumentParser(description='BX v118 prev-link quarantine branch splitter')
    sub = ap.add_subparsers(dest='cmd', required=True)

    p = sub.add_parser('split-prevlink-bucket')
    p.add_argument('tng_path', type=Path)
    p.add_argument('out_dir', type=Path)
    p.add_argument('--sig7', type=str, default='0000010c425425')
    p.add_argument('--prev', type=str, default='0B@-295')
    ...
    ns = ap.parse_args()
    if ns.cmd != 'split-prevlink-bucket':
        raise SystemExit(1)
    ...
    summary = []
    summary.append('BX v118 prev-link quarantine branch splitter')
    summary.append('===========================================')
    ...
    for row in stable_rows:
        proposed_rules.append({
            'name': f'prevlink_295_{row["sig8"][-2:]}_{row["body_prefix"]}',
            'sig7': ns.sig7,
            'prev': ns.prev,
            'next': ns.next,
            'member': {
