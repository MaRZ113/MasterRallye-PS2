...
def build_backbone_rules(quarantine_rows, sig7: str, min_count: int):
    # only split target backbone bucket none||none
    ...
    for (sig8, body_prefix), count in branch_counts.items():
        if count >= min_count:
            rules.append({
                ...
                'next': 'none',
                'members': {
                    sig8: {'body_prefix': body_prefix}
                },
                'count': count,
                'source': 'auto_backbone_promoted',
            })
...
def main():
    ap = argparse.ArgumentParser(description='BX v125 backbone-aware 425425 framework extractor')
    sub = ap.add_subparsers(dest='cmd', required=True)

    p = sub.add_parser('extract-425425-framework-backbone')
    ...
    ns = ap.parse_args()
    if ns.cmd != 'extract-425425-framework-backbone':
        raise SystemExit(1)
    ...
    # Phase 2: backbone auto-promotion from none||none
    auto_backbone_rules = build_backbone_rules(base_quarantine, ns.sig7, ns.min_backbone_count)
    full_registry = BASE_BUCKET_REGISTRY + auto_backbone_rules

    # Phase 3: final assignment
    framework_rows, quarantine_rows = assign_hits(all_hits, full_registry)
    ...
    # Summary
    summary = []
    summary.append('BX v125 backbone-aware 425425 framework extractor')
    summary.append('===============================================')
    ...
    summary.append(f'assigned_hits_after_backbone: {len(framework_rows)}')
    summary.append(f'quarantine_hits_after_backbone: {len(quarantine_rows)}')
    summary.append('')

    if auto_backbone_rules:
        summary.append('Auto-promoted backbone rules:')
        for rule in auto_backbone_rules[:50]:
    ...
    for name, count in bucket_counts.most_common(30):
        summary.append(f'  {name} :: {count}')
    summary.append('')

    q_bucket_counts = Counter(f'{r["prev_key"]} || {r["next_key"]}' for r in quarantine_rows)
    summary.append('Top remaining quarantine buckets:')
    for name, count in q_bucket_counts.most_common(30):
        summary.append(f'  {name} :: {count}')
    ...
    (out_dir / 'meta.json').write_text(json.dumps({
        ...
        'assigned_hits_after_backbone': len(framework_rows),
        'quarantine_hits_after_backbone': len(quarantine_rows),
