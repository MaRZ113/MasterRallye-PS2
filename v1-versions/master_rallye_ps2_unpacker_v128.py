...
def main():
    ap=argparse.ArgumentParser(description='BX v128 secondary frontier layer for 425425**')
    sub=ap.add_subparsers(dest='cmd', required=True)

    p=sub.add_parser('extract-425425-framework-secondary')
    ...
    ns=ap.parse_args()
    if ns.cmd != 'extract-425425-framework-secondary':
        raise SystemExit(1)
    ...
    # wave 4: micro-pure secondary layer from remaining quarantine, excluding already-covered bucket keys
    ...
    for bucket, hits in bucket_hits.items():
        ...
        frontier_rows.append({
            ...
            'action_hint': 'split_dominant' if top_count >= 2 and top_count/len(hits) >= 0.5 else 'deep_noise',
        })
    ...
    summary=[]
    summary.append('BX v128 secondary frontier layer for 425425**')
    summary.append('============================================')
    ...
    summary.append(f'after_micro_pure_assigned_hits: {len(final_framework)}')
    ...
    summary.append('')
    if micro_pure_rules:
        summary.append('Micro-pure promoted rules:')
        for rule in micro_pure_rules[:40]:
        ...
        summary.append('')
    summary.append('Top split-dominant frontier buckets:')
    for row in split_rows[:30]:
        summary.append(
            f'  {row["bucket"]}: hits={row["hits"]} branches={row["unique_branches"]} '
            f'top={row["top_branch"]}::{row["top_count"]} dominance={row["dominance"]}'
        )
    summary.append('')
    summary.append('Top deep-noise buckets:')
    for row in noise_rows[:30]:
        summary.append(
            f'  {row["bucket"]}: hits={row["hits"]} branches={row["unique_branches"]} '
            f'top={row["top_branch"]}::{row["top_count"]} dominance={row["dominance"]}'
        )
    ...
    (out_dir/'meta.json').write_text(json.dumps({
        ...
        'after_pure_assigned_hits': len(wave2_framework),
        'after_backbone_assigned_hits': len(wave3_framework),
        'after_micro_pure_assigned_hits': len(final_framework),
        ...
        'micro_pure_rules': len(micro_pure_rules),
