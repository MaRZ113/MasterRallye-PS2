...
def build_dominant_split_rules(quarantine_rows, sig7: str, min_count: int, min_dominance: float):
    bucket_hits=defaultdict(list)
    for hit in quarantine_rows:
        bucket_hits[(hit['prev_key'], hit['next_key'])].append(hit)
    rules=[]; bucket_rows=[]
    for (prev_key,next_key), hits in bucket_hits.items():
        ...
        if len(branches) >= 2 and top_count >= min_count and dominance >= min_dominance:
            hit0 = next(h for h in hits if branch_key(h) == top_branch)
            rules.append(build_auto_rule(sig7, prev_key, next_key, hit0, top_count, 'auto_dominant_split'))
    rules.sort(key=lambda r:(-r['count'],r['name']))
...
def main():
    ap=argparse.ArgumentParser(description='BX v130 dominant-wave 425425 framework extractor')
    sub=ap.add_subparsers(dest='cmd', required=True)

    p=sub.add_parser('extract-425425-framework-dominant-wave')
    ...
    ns=ap.parse_args()
    if ns.cmd != 'extract-425425-framework-dominant-wave':
        raise SystemExit(1)
    ...
    # unified waves through micro-pure
    ...
    # dominant wave
    dominant_rules, dominant_bucket_rows = build_dominant_split_rules(wave4_quarantine, ns.sig7, ns.min_dominant_count, ns.min_dominance)
    final_registry = reg4 + dominant_rules
    ...
    write_csv(out_dir/'dominant_bucket_candidates.csv', dominant_bucket_rows,
    ...
    summary=[]
    summary.append('BX v130 dominant-wave 425425 framework extractor')
    summary.append('===============================================')
    ...
    summary.append(f'after_backbone_assigned_hits: {len(wave3_framework)}')
    summary.append(f'after_micro_pure_assigned_hits: {len(wave4_framework)}')
    summary.append(f'after_dominant_wave_assigned_hits: {len(final_framework)}')
    ...
    summary.append(f'min_dominant_count: {ns.min_dominant_count}')
    summary.append(f'min_dominance: {ns.min_dominance}')
    summary.append('')
    if dominant_rules:
        summary.append('Dominant-wave promoted rules:')
        for rule in dominant_rules[:40]:
    ...
    (out_dir/'meta.json').write_text(json.dumps({
        ...
        'after_micro_pure_assigned_hits': len(wave4_framework),
        'after_dominant_wave_assigned_hits': len(final_framework),
        'remaining_quarantine_hits': len(final_quarantine),
        'dominant_split_rules': len(dominant_rules),
