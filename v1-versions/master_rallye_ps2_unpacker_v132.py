...
def build_dominant_split_rules(quarantine_rows, sig7: str, min_count: int, min_dominance: float, exact_bucket_size: int | None, require_two_branches: bool):
    bucket_hits=defaultdict(list)
    for hit in quarantine_rows:
        bucket_hits[(hit['prev_key'], hit['next_key'])].append(hit)
    rules=[]; bucket_rows=[]
    for (prev_key,next_key), hits in bucket_hits.items():
        ...
        cond_branches = (len(branches) == 2) if require_two_branches else (len(branches) >= 2)
        if cond_branches and top_count >= min_count and dominance >= min_dominance and cond_size:
            hit0 = next(h for h in hits if branch_key(h) == top_branch)
            rules.append(build_auto_rule(sig7, prev_key, next_key, hit0, top_count, 'triad_dominant_wave'))
    rules.sort(key=lambda r:(-r['count'],r['name']))
...
def main():
    ap=argparse.ArgumentParser(description='BX v132 triad-dominant wave extractor')
    sub=ap.add_subparsers(dest='cmd', required=True)

    p=sub.add_parser('extract-425425-framework-triad-dominant')
    ...
    ns=ap.parse_args()
    if ns.cmd != 'extract-425425-framework-triad-dominant':
        raise SystemExit(1)
    ...
    dominant_rules, _ = build_dominant_split_rules(wave4_quarantine, ns.sig7, ns.min_dominant_count, ns.min_dominance, None, False)
    reg5 = reg4 + dominant_rules
    wave5_framework, wave5_quarantine = assign_hits(all_hits, reg5)

    # triad-dominant wave: exactly 3 hits, exactly 2 branches, top 2/3
    triad_rules, triad_candidates = build_dominant_split_rules(
        wave5_quarantine,
        ns.sig7,
        ns.triad_min_dominant_count,
        ns.triad_min_dominance,
        3,
        True
    )
    final_registry = reg5 + triad_rules
    ...
    write_csv(out_dir/'triad_dominant_candidates.csv', triad_candidates,
    ...
    with (out_dir/'triad_dominant_rules.json').open('w', encoding='utf-8') as f:
        json.dump(triad_rules, f, indent=2)
    with (out_dir/'bucket_registry.json').open('w', encoding='utf-8') as f:
        json.dump(final_registry, f, indent=2)

    summary=[]
    summary.append('BX v132 triad-dominant wave extractor')
    summary.append('====================================')
    summary.append(f'total_hits_under_sig7: {len(all_hits)}')
    summary.append(f'after_primary_dominant_wave: {len(wave5_framework)}')
    summary.append(f'after_triad_dominant_wave: {len(final_framework)}')
    ...
    summary.append(f'triad_dominant_rules: {len(triad_rules)}')
    summary.append('')
    if triad_rules:
        summary.append('Triad-dominant promoted rules:')
        for rule in triad_rules[:40]:
    ...
    (out_dir/'meta.json').write_text(json.dumps({
        ...
        'after_triad_dominant_wave': len(final_framework),
        'remaining_quarantine_hits': len(final_quarantine),
        'triad_dominant_rules': len(triad_rules),
