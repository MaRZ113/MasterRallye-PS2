...
def main():
    ap=argparse.ArgumentParser(description='BX v133 micro-backbone wave extractor')
    sub=ap.add_subparsers(dest='cmd', required=True)

    p=sub.add_parser('extract-425425-framework-micro-backbone')
    ...
    ns=ap.parse_args()
    if ns.cmd != 'extract-425425-framework-micro-backbone':
        raise SystemExit(1)
    ...
    auto_backbone_rules = build_backbone_rules(wave2_quarantine, ns.sig7, ns.min_backbone_count, 'auto_backbone')
    ...
    dominant_rules, _ = build_dominant_split_rules(wave4_quarantine, ns.sig7, ns.min_dominant_count, ns.min_dominance, None, 'auto_dominant_split')
    reg5 = reg4 + dominant_rules
    wave5_framework, wave5_quarantine = assign_hits(all_hits, reg5)
    triad_rules, _ = build_dominant_split_rules(wave5_quarantine, ns.sig7, ns.triad_min_dominant_count, ns.triad_min_dominance, ns.triad_exact_bucket_size, 'triad_dominant_wave')
    reg6 = reg5 + triad_rules
    wave6_framework, wave6_quarantine = assign_hits(all_hits, reg6)

    # micro-backbone on remaining none||none only, count>=2
    micro_backbone_rules = build_backbone_rules(wave6_quarantine, ns.sig7, ns.micro_backbone_min_count, 'micro_backbone')
    final_registry = reg6 + micro_backbone_rules
    ...
    with (out_dir/'micro_backbone_rules.json').open('w', encoding='utf-8') as f:
        json.dump(micro_backbone_rules, f, indent=2)
    with (out_dir/'bucket_registry.json').open('w', encoding='utf-8') as f:
        json.dump(final_registry, f, indent=2)

    summary=[]
    summary.append('BX v133 micro-backbone wave extractor')
    summary.append('===================================')
    summary.append(f'total_hits_under_sig7: {len(all_hits)}')
    summary.append(f'after_triad_dominant_wave: {len(wave6_framework)}')
    summary.append(f'after_micro_backbone_wave: {len(final_framework)}')
    summary.append(f'remaining_quarantine_hits: {len(final_quarantine)}')
    summary.append(f'micro_backbone_rules: {len(micro_backbone_rules)}')
    summary.append('')
    if micro_backbone_rules:
        summary.append('Micro-backbone promoted rules:')
        for rule in micro_backbone_rules[:60]:
            sig8, member = next(iter(rule['members'].items()))
            summary.append(f'  {rule["name"]}: {sig8} | {member["body_prefix"]} :: {rule["count"]}')
        summary.append('')
    ...
    (out_dir/'meta.json').write_text(json.dumps({
        ...
        'after_triad_dominant_wave': len(wave6_framework),
        'after_micro_backbone_wave': len(final_framework),
        'remaining_quarantine_hits': len(final_quarantine),
        'micro_backbone_rules': len(micro_backbone_rules),
